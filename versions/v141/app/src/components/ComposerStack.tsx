...
interface ComposerStackProps {
  ...
  onEnterRealtimeMode: () => void
  onExitRealtimeMode: () => void
...
const ComposerStack = memo(forwardRef<ComposerStackHandle, ComposerStackProps>(function ComposerStack({
  ...
  const canSendComposer = !isRealtimeMode && (hasInputText || pendingImages.length > 0)
  const docParsingInProgress = docUploads.length > 0
  const primaryAction: 'send' | 'voice' | 'close' = canSendComposer ? 'send' : (isVoiceMode || isRealtimeMode) ? 'close' : 'voice'
    ...
    if (primaryAction === 'close') {
      return (
        <Button
          ...
          data-state="close"
          icon={<CloseOutlined />}
          onClick={isRealtimeMode ? onExitRealtimeMode : onExitVoiceMode}
          aria-label={isRealtimeMode ? '退出实时对话' : '退出语音模式'}
        />
      )
    }
    return (
      ...
      <div className="composer-stack">
        ...
        {isRealtimeMode && (
          <div className={`realtime-composer-status realtime-phase-${realtimePhase}`} data-testid="realtime-composer-status" aria-live="polite">
            ...
            <Button type="text" shape="circle" size="small" icon={<CloseOutlined />} onClick={onExitRealtimeMode} aria-label="退出实时对话" />
          </div>
        )}
        <div className="composer-input-shell">
          ...
          <Input.TextArea
            ...
            placeholder={isRealtimeMode ? '实时对话中，退出后可输入文字' : '给罗宾发送消息'}
            disabled={isRealtimeMode || historySyncState === 'syncing' || isTranscribing}
            aria-label="文字消息"
          />
