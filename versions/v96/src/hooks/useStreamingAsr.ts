...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const applyLiveText = useCallback((pieces: string[]) => {
    ...
    const showText = bestInterimRef.current
    if (!showText || showText === interimTextRef.current) return false

    committedTextRef.current = ''
    hypothesisTextRef.current = showText
    interimTextRef.current = showText
    ...
    onInterimTextRef.current(showText)
    return true
  }, [emitDebugStats])

  const markStreamLiveActive = useCallback(() => {
  ...
  const requestWebmPreview = useCallback((blob: Blob, urgent = false) => {
    ...
    void transcribeAudio(previewBlob, controller.signal).then((text) => {
      ...
      if (trimmed) applyLiveText([trimmed])
    ...
    uploadQueueRef.current = uploadQueueRef.current.catch(() => undefined).then(async () => {
      while (
        ...
        try {
          ...
          if (hadStreamText) {
            applyLiveText(pieces)
            debugStatsRef.current.streamTextHits += 1
            markStreamLiveActive()
          }
          emitDebugStats()
        } catch {
          pendingChunksRef.current.unshift(chunk)
  ...
  }, [applyLiveText, armIdleCleanup, emitDebugStats, markStreamLiveActive, requestWebmPreview])
