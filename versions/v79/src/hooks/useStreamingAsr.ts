...
/** webm 增量片合并后再上传 asr-stream（百度 MID 需足够 webm 体积） */
const webmStreamFirstMinBytes = 2_500
const webmStreamMinBytes = 8_000
const webmStreamFirstWaitMs = 350
const webmStreamWaitMs = 550
...
/** 跟嘴专用：录音中定时 webm /asr（chunk 无 MID 时补跟嘴） */
const livePreviewLoopMs = 550
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  /** 百度 MID 已命中：停用 webm /asr 预览，避免覆盖跟嘴 */
  ...
  const emptyChunkUploadsRef = useRef(0)
  ...
  const resetSessionState = useCallback((clearDisplay = false) => {
    ...
    emptyChunkUploadsRef.current = 0
  ...
  const applyStreamPieces = useCallback((pieces: string[], fromStream = false) => {
  ...
  const pumpUploads = useCallback(() => {
    ...
    uploadQueueRef.current = uploadQueueRef.current.catch(() => undefined).then(async () => {
      while (
        ...
        else if (blob.type.includes('webm')) webmStreamSentCountRef.current += 1
        ...
        try {
          ...
          if (hadStreamText) {
            ...
            emptyChunkUploadsRef.current += 1
            if (
              emptyChunkUploadsRef.current >= 2
  ...
  const flushStreamWebmBuffer = useCallback(() => {
    ...
    streamWebmBufferRef.current = []
    streamWebmBytesRef.current = 0
    pendingChunksRef.current.push(new Blob(slices, { type: 'audio/webm' }))
    pumpUploads()
  }, [clearChunkTimer, pumpUploads])

  const enqueueChunk = useCallback((blob: Blob) => {
    if (!blob.size) return
    // 百度 VPS：asr-stream/chunk 用 webm 增量片；PCM 块不返 MID
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    let finalSource: StreamingAsrResult['finalSource'] = finalText.trim()
      ? 'baidu-end'
      : (streamWorked && chosen.text ? 'baidu-stream' : '')
    ...
    // 流式未出字时才用整段 /asr 收尾（百度 MID 已工作时以 end+interim 为准）
    if (fullRecording.size >= finalAsrMinBytes && !streamWorked) {
      try {
    ...
    } else if (streamWorked && chosen.text) {
      chosen = {
        text: correctAsrText(collapseRepeatedPhrases(chosen.text)),
        usedFallback: chosen.usedFallback,
      }
