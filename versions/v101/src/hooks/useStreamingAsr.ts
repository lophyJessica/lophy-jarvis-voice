...
function applyStreamPiece(
  ...
  // 百度多句朗读：新句 MID 与旧句不相似时接在后面，勿整段替换丢前文
  const prior = displayed
  if (!prior) return { committed: '', hypothesis: incoming }
  if (incoming.startsWith(prior)) return { committed: '', hypothesis: incoming }
  const nextCommitted = hypothesis
    ? (committed.endsWith(hypothesis) ? committed : `${committed}${hypothesis}`)
    : committed
  const base = `${nextCommitted}` || prior
  const sep = /[，。！？、；：,\.!?;:\s]$/.test(base) ? '' : '，'
  return { committed: '', hypothesis: `${base}${sep}${incoming}` }
}
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const applyStreamPieces = useCallback((pieces: string[]) => {
    for (const piece of pieces) {
      ...
      bestInterimRef.current = correctAsrText(
        mergeInterimText(bestInterimRef.current, rawClean),
      )
    }
    ...
    bestInterimRef.current = correctAsrText(
      mergeInterimText(bestInterimRef.current, nextText),
    )
    if (nextText === interimTextRef.current) return false
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    if (fullRecording.size >= 6_000) {
      try {
        ...
        if (fallbackText) {
          const merged = chooseFinalText(endClean, fallbackText, bestInterimText || interimText)
          ...
          if (
            merged.text.length > chosen.text.length
          ...
          } else if (streamCollapsed.length > chosen.text.length) {
