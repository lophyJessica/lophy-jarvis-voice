...
export type RealtimeVoicePhase = 'idle' | 'connecting' | 'connected' | 'thinking' | 'speaking' | 'error'
...
interface RealtimeVoiceOptions {
  thinkingPromptAudioRef?: { current: HTMLAudioElement | null }
  thinkingPromptPlayingRef?: { current: boolean }
}
...
function eventType(payload: RealtimeEvent) {
  const directType = payload.type ?? payload.event
  if (typeof directType === 'string') return directType.trim().toLowerCase()
  if (payload.data && typeof payload.data === 'object') {
    const nested = payload.data as Record<string, unknown>
    const nestedType = nested.type ?? nested.event
    if (typeof nestedType === 'string') return nestedType.trim().toLowerCase()
  }
  return ''
}

function isThinkingEvent(payload: RealtimeEvent) {
  const type = eventType(payload)
  return type === 'thinking' || type.endsWith('.thinking') || type.includes('thinking')
}

function eventText(payload: RealtimeEvent) {
  const value = eventValue(payload, ['text', 'transcript', 'content', 'recognized_text', 'recognition', 'asr_text'])
  return value == null ? '' : String(value).trim()
}

function eventRawText(payload: RealtimeEvent) {
  const value = eventValue(payload, ['text', 'content'])
  return value == null ? '' : String(value)
}

export function useRealtimeVoice({ thinkingPromptAudioRef, thinkingPromptPlayingRef }: RealtimeVoiceOptions = {}) {
  const [state, setState] = useState<RealtimeVoiceState>({ phase: 'idle', transcript: '', robinText: '', error: null })
  ...
  const isThinkingPromptPlaying = useCallback(() => {
    if (thinkingPromptPlayingRef?.current) return true
    const audio = thinkingPromptAudioRef?.current
    return Boolean(audio && !audio.paused && !audio.ended)
  }, [thinkingPromptAudioRef, thinkingPromptPlayingRef])

  const playPcm = useCallback((frame: ArrayBuffer) => {
    if (isThinkingPromptPlaying()) return
    const context = playbackContextRef.current
    ...
    setState((current) => current.phase === 'connected' || current.phase === 'thinking' ? { ...current, phase: 'speaking' } : current)
  }, [isThinkingPromptPlaying])
  ...
  const stop = useCallback(() => {
    ...
    setState({ phase: 'idle', transcript: '', robinText: '', error: null })
  }, [stopPlayback])
  ...
  const start = useCallback(async () => {
    ...
    setState({ phase: 'connecting', transcript: '', robinText: '', error: null })
    ...
    try {
      ...
      socket.onmessage = (event) => {
        ...
        if (event.data instanceof ArrayBuffer) {
          if (!isThinkingPromptPlaying()) playPcm(event.data)
          return
        }
        if (event.data instanceof Blob) {
          if (isThinkingPromptPlaying()) return
          void event.data.arrayBuffer().then(playPcm)
          return
        }
        ...
        try {
          ...
          if (isThinkingEvent(payload)) {
            // 清掉 thinking 事件到达前可能已排队的回复帧，提示音期间不与旧回复叠加。
            stopPlayback()
            const thinkingText = eventText(payload)
            setState((current) => ({
              ...current,
              phase: 'thinking',
              transcript: thinkingText || current.transcript,
              robinText: '',
              error: null,
            }))
            return
          }
          if (eventType(payload) === 'robin_text') {
            const robinText = eventRawText(payload)
            if (!robinText.trim()) return
            setState((current) => ({
              ...current,
              robinText: current.robinText + robinText,
              error: null,
            }))
            return
          }
          const text = eventText(payload)
          if (text) setState((current) => ({
            ...current,
            // ASR/文本事件一到就给出明确的思考反馈；若刚被打断且仍在播放态，也要切回思考态。
            phase: current.phase === 'connected' || current.phase === 'speaking' ? 'thinking' : current.phase,
            transcript: text,
          }))
        } catch { return }
      }
      ...
      socket.onclose = () => {
        if (!activeRef.current || sessionRef.current !== sessionId) return
        stop()
        setState({ phase: 'error', transcript: '', robinText: '', error: '连接断开，请退出后重试' })
      }
      ...
      setState({ phase: 'error', transcript: '', robinText: '', error: error instanceof Error ? error.message : '实时语音启动失败' })
      throw error
    }
  }, [isThinkingPromptPlaying, playPcm, stop, stopPlayback])
