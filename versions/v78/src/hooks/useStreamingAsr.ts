...
/** PCM：与 worklet 10_240 字节批次对齐 */
const pcmUploadBufferMs = 80
const pcmFirstUploadDelayMs = 0
/** 累积 webm 达到此体积才作为 asr-stream chunk（百度 MID 路径，对齐 v8） */
const webmStreamMinBytes = 4_000
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const enqueueChunk = useCallback((blob: Blob) => {
    if (!blob.size) return
    // 有可用 webm 快照时改发累积 webm，避免超短 PCM 块导致 chunk text 恒空
    if (blob.type === 'audio/pcm' && (latestWebmRef.current?.size ?? 0) >= webmStreamMinBytes) return
  ...
  const enqueueWebmSnapshot = useCallback((blob: Blob) => {
    ...
    if (blob.size >= webmStreamMinBytes) enqueueChunk(blob)
  }, [enqueueChunk])
