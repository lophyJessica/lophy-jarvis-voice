...
const webmPreviewIntervalMs = 800
const webmPreviewIntervalWithInterimMs = 1_800
const webmPreviewMinBytes = 1_500
...
export function useStreamingAsr({ onInterimText }: UseStreamingAsrOptions) {
  ...
  const requestWebmPreview = useCallback((blob: Blob, urgent = false) => {
    if (!blob.size || blob.size < webmPreviewMinBytes) return
    ...
    if (
      !urgent
      && (webmPreviewInFlightRef.current || now - lastWebmPreviewAtRef.current < minInterval)
    ) return
  ...
  }, [applyStreamPieces, armIdleCleanup, requestWebmPreview])
