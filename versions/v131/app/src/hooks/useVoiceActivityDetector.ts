...
/** v54：持续有声 ~150ms 再上传预滚动（会话已在第一帧有声时创建） */
const defaultSpeechPrimeDelayMs = 150
/** 开录须晚于 prime（v54 280ms） */
const defaultSpeechStartDelayMs = 280
/**
 * 静音多久才判定「说完了」并分段发送。
...
/** 跟嘴：webm 切片间隔（过短易导致首包 /asr 空返、拖慢首字） */
const recorderTimesliceMs = 250
/**
 * 音量在阈值附近抖动时不要立刻撤销预热。
 * 必须持续安静这么久，才允许下一次开口重新 prime（否则会疯狂新建 ASR 会话）。
 */
const primeResetSilenceMs = 600
...
export function useVoiceActivityDetector({
  ...
  const lastVolumeValueRef = useRef(0)
  ...
  const startRecorder = useCallback(() => {
    ...
    if (!mimeType) {
      setError('当前环境不支持可用的音频编码格式')
      return false
    }

    const recorder = new MediaRecorder(stream, { mimeType })
    const usePcmStreaming = Boolean(pcmWorkletRef.current) && !preferWebmStreamingCapture()
    chunksRef.current = []
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      const skipAudioWorklet = preferWebmStreamingCapture()
      if (
        !skipAudioWorklet
        && pendingAudioContext.audioWorklet
    ...
    const tick = () => {
      ...
      // UI 音量节流：避免每帧 setState 拖垮输入框（APK WebView 更敏感）
      if (
        now - lastVolumeUpdateRef.current >= 160
        && Math.abs(nextVolume - lastVolumeValueRef.current) >= 0.008
      ) {
        lastVolumeUpdateRef.current = now
        lastVolumeValueRef.current = nextVolume
        startTransition(() => setVolume(nextVolume))
      }
      ...
      if (hasVoice) {
        ...
        if (!isVoiceActiveRef.current) {
          ...
          // 音节间隙（<600ms）不撤销预热，避免一次开口反复 prime 出多个 ASR 会话
          if (now - quietSinceRef.current >= primeResetSilenceMs) {
            speechPrimeStartedAtRef.current = null
            speechSessionStartedRef.current = false
            if (speechPrimedRef.current) {
              speechPrimedRef.current = false
