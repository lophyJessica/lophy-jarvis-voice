...
import { isCapacitorNative, preferWebmStreamingCapture } from '../utils/platform'
...
const apkWebmPreviewIntervalMs = 900
const apkWebmPreviewFirstWaitMs = 650
const apkWebmPreviewMinBytes = 1_600
const apkWebmPreviewFirstMinBytes = 800
/** 自上次预览后字节增长达到此值也可提前触发（不只等时间） */
const apkWebmPreviewGrowthBytes = 2_800
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const lastWebmPreviewBytesRef = useRef(0)
  ...
  const resetSessionState = useCallback((clearDisplay = false) => {
    ...
    lastWebmPreviewBytesRef.current = 0
   ...
   * 上一次未返回前不发新请求；时间间隔约 0.65s/0.9s，或字节增长足够时提前触发。
   */
  const requestApkWebmPreview = useCallback((blob: Blob) => {
    ...
    const grewEnough = !awaitingFirst
      && blob.size - lastWebmPreviewBytesRef.current >= apkWebmPreviewGrowthBytes
    if (!awaitingFirst && !grewEnough && now - lastWebmPreviewAtRef.current < minInterval) return

    lastWebmPreviewAtRef.current = now
    lastWebmPreviewBytesRef.current = blob.size
    webmPreviewInFlightRef.current = true
    const controller = new AbortController()
    webmPreviewControllerRef.current = controller
    const generation = generationRef.current
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    if (isCapacitorNative()) {
      ...
      const webmCandidates = [fullRecording, latestWebmRef.current].filter(
        (blob): blob is Blob => Boolean(blob && blob.size > 0),
      )
      ...
      if (recordingBlob.size > 0) {
        try {
          const raw = (await transcribeAudio(recordingBlob)).trim()
          text = correctAsrText(collapseRepeatedPhrases(raw))
        } catch {
          text = ''
        }
      }
      ...
      if (text) {
        ...
        onInterimTextRef.current(text)
      }
      resetSessionState(false)
      return {
        text,
        usedFallback: true,
        finalSource: text ? 'fallback-asr' : '',
      }
    }
    ...
    return { text: '', usedFallback: true, finalSource: '' }
  }, [clearChunkTimer, clearIdleTimer, flushPendingChunks, flushStreamWebmBuffer, resetSessionState, stopLivePreviewLoop])
