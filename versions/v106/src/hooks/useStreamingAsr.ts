...
/** 长句：webm 补全间隔（chunk 跟嘴后、停滞时再打 /asr） */
const longReadWebmIntervalMs = 5_500
const longReadWebmFirstDelayMs = 5_000
const longReadWebmMinChars = 72
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const publishLiveText = useCallback((fromChunk = false) => {
    const nextText = correctAsrText(collapseRepeatedPhrases(pickLongestMerged(
      liveAccumRef.current,
      bestInterimRef.current,
      peakRawMidRef.current,
    )))
  ...
  const applyStreamPieces = useCallback((pieces: string[]) => {
    for (const piece of pieces) {
      ...
      liveAccumRef.current = mergeChunkMid(liveAccumRef.current, rawClean)
      bestInterimRef.current = mergeChunkMid(bestInterimRef.current, rawClean)
    }
  ...
  const beginLivePreview = useCallback(() => {
    ...
    const tick = () => {
      ...
      if (
        ...
        && (chars < longReadWebmMinChars || stagnantStreamHitsRef.current >= 3)
      ) {
