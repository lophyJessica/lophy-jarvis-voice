...
const webmPreviewGrowthBytes = 400
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const requestWebmPreview = useCallback((blob: Blob, urgent = false) => {
    ...
    void transcribeAudio(previewBlob, controller.signal).then((text) => {
      ...
      if (!trimmed) {
        ...
        if (latest && latest.size > previewBlob.size + 200) {
  ...
  const tryScheduleWebmPreview = useCallback((urgent = false) => {
    ...
    if (awaitingFirst) {
      if (blob.size < webmPreviewMinBytes) return
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    if (fullRecording.size >= finalAsrMinBytes) {
      try {
        tailWebmText = correctAsrText(collapseRepeatedPhrases((await transcribeAudio(fullRecording)).trim()))
        if (tailWebmText) applyPreviewText(tailWebmText)
      } catch {
        // 整段 webm 失败时仍走 end + 已有预览
      }
    }
    ...
    if (tailWebmText.length > chosen.text.length) {
      chosen = chooseFinalText(finalText.trim(), tailWebmText, bestInterimText)
    }
    let finalSource: StreamingAsrResult['finalSource'] = finalText.trim()
      ? 'baidu-end'
      : (chosen.text ? 'webm-preview' : '')
    if (tailWebmText && chosen.text === correctAsrText(collapseRepeatedPhrases(tailWebmText))) {
    ...
    if (!chosen.text && fullRecording.size >= finalAsrMinBytes && !tailWebmText) {
      try {
        chosen = {
  ...
  }, [applyPreviewText, clearIdleTimer, flushPendingChunks, resetSessionState, stopLivePreviewLoop])
