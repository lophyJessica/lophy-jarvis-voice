...
function mergeInterimText(current: string, incoming: string): string {
  ...
  // 与全文或尾部很像：视为同句修订，替换而不是追加（避免 v37 重复乱码）
  ...
  if (fullSim >= 0.55 || tailSim >= 0.6) {
    if (tailSim >= fullSim && current.length > incoming.length) {
      const cut = Math.max(0, current.length - tail.length)
      return `${current.slice(0, cut)}${incoming}`
    }
...
function chooseFinalText(finalText: string, interimText: string): { text: string; usedFallback: boolean } {
  ...
  if (finalText && interimText) {
    const merged = clean(mergeInterimText(interimText, finalText))
    const finalClean = clean(finalText)
    const interimClean = clean(interimText)
    // end 过短或明显不如 interim 完整时，用合并/interim
    if (finalClean.length < interimClean.length * 0.6) {
      const best = merged.length >= interimClean.length ? merged : interimClean
      return { text: best, usedFallback: true }
    }
    if (merged.length > finalClean.length * 1.15 && merged.length >= interimClean.length) {
      return { text: merged, usedFallback: true }
    }
    return { text: finalClean.length >= interimClean.length ? finalClean : interimClean, usedFallback: finalClean.length < interimClean.length }
  }
  if (finalText) return { text: clean(finalText), usedFallback: false }
  if (interimText) return { text: clean(interimText), usedFallback: true }
