...
function VoiceConsole({ username, onLogout, isDev }: { username: string; onLogout: () => void; isDev: boolean }) {
  ...
  const [vadThreshold, setVadThreshold] = useState(0.018)
  ...
  const handleVadSpeechPrime = useCallback(() => {
    // 声音刚起就创建 ASR 会话，避免等开录后再 round-trip 吃掉句首
    ...
    startStreamingAsr()
  }, [cancelSpeech, startStreamingAsr])
  ...
  const {
    ...
    onSpeechPrimeCancel: cleanupStreamingAsr,
