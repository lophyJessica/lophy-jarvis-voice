...
/** 默认音量阈值：略低更灵敏，轻声朗读/换气不易被当成静音。 */
const defaultSilenceThreshold = 0.018
/** 开录确认窗：过长会吃掉句首；预滚动 PCM 已覆盖判定前音频。 */
const defaultSpeechStartDelayMs = 80
/**
 * 静音多久才判定「说完了」并分段发送。
...
export function useVoiceActivityDetector({
  ...
  }, [canStartSpeech, onRecordingChunk, onRecordingComplete, onSpeechEnd, onSpeechPrime, onSpeechPrimeCancel, onSpeechStart])
  ...
  const startRecorder = useCallback(() => {
    ...
    recorder.ondataavailable = (event) => {
      if (event.data.size > 0) {
        ...
        if (streamingChunkModeRef.current === 'webm') callbacksRef.current.onRecordingChunk?.(event.data)
      }
    }
    recorder.onerror = () => {
      chunksRef.current = []
    ...
    if (usePcmStreaming) pcmWorkletRef.current?.port.postMessage({ type: 'start' })
    setIsRecording(true)
    return true
  }, [])
  ...
  const start = useCallback(async () => {
    ...
    if (pendingPcmWorklet) {
      pendingPcmWorklet.port.onmessage = (event: MessageEvent<{ type?: string; buffer?: ArrayBuffer }>) => {
        if (event.data?.type === 'pcm' && event.data.buffer && streamingChunkModeRef.current === 'pcm') {
    ...
    const tick = () => {
      ...
      if (hasVoice) {
        ...
        if (!isVoiceActiveRef.current && speechPrimedRef.current) {
          speechPrimedRef.current = false
