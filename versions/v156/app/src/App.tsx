...
import { ROBIN_AVATAR_URL } from './version'
...
function getLatestAssistantMessageText(messages: StoredMessage[]) {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const message = messages[index]
    if (message?.role === 'assistant') return getMessageText(message.content)
  }
  return ''
}
...
function VoiceConsole({ username, onLogout, isDev }: { username: string; onLogout: () => void; isDev: boolean }) {
  ...
  const replayableSpeechText = useMemo(
    () => cleanSpeechText(getLatestAssistantMessageText(messages)),
    [messages],
  )
