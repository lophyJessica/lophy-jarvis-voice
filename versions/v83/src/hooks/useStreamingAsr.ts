...
const webmPreviewIntervalMs = 300
const webmPreviewIntervalWithInterimMs = 580
const webmPreviewMinBytes = 700
const webmPreviewFirstMinBytes = 240
/** 累积 webm 增长超过此值且上一轮预览已结束 → 立刻再打 /asr */
const webmPreviewGrowthBytes = 320
const livePreviewLoopMs = 360
const livePreviewFirstDelayMs = 0
const livePreviewEarlyProbeMs = 60
const livePreviewSecondProbeMs = 150
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const requestWebmPreview = useCallback((blob: Blob, urgent = false) => {
    ...
    if (!urgent && now - lastWebmPreviewAtRef.current < minInterval) return false
    ...
    void transcribeAudio(previewBlob, controller.signal).then((text) => {
      ...
      if (
        ...
        pendingPreviewRef.current = false
        requestWebmPreview(latest, lastWebmPreviewAtRef.current === 0)
      }
    }).catch(() => {
  ...
  const tryScheduleWebmPreview = useCallback((urgent = false) => {
    ...
    if (!isFirst && blob.size <= lastPreviewBlobSizeRef.current + webmPreviewGrowthBytes) return
    requestWebmPreview(blob, urgent || isFirst)
  }, [requestWebmPreview])
  ...
  const beginLivePreview = useCallback(() => {
    ...
    window.setTimeout(() => {
      if (livePreviewGenerationRef.current !== generationRef.current) return
      if (lastWebmPreviewAtRef.current === 0) tryScheduleWebmPreview(true)
    }, livePreviewSecondProbeMs)
  ...
  const enqueueWebmSnapshot = useCallback((blob: Blob) => {
    ...
    if (blob.size >= webmPreviewFirstMinBytes) {
      tryScheduleWebmPreview(lastWebmPreviewAtRef.current === 0)
    }
  }, [tryScheduleWebmPreview])
