...
export function useStreamingAsr({ onInterimText }: UseStreamingAsrOptions) {
  ...
  /** chunk 无中间 text 时（如腾讯仅 end 出字），用 webm 定期整段 /asr 补跟嘴 */
  const enqueueWebmPreview = useCallback((blob: Blob) => {
    if (!blob.size || blob.size < 3_000) return
    const now = performance.now()
    if (webmPreviewInFlightRef.current || now - lastWebmPreviewAtRef.current < 2_200) return
  // 流式已有较长文本时不再抢带宽
    if (interimTextRef.current.length > 16 && !streamFailedRef.current) return
