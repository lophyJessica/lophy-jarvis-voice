...
export function useStreamingAsr({ onInterimText }: UseStreamingAsrOptions) {
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    // 录音不短但识别结果过短：再走整段 /asr，补回被流式窗口丢掉的前文
    const recordingLooksLong = fullRecording.size >= 60_000
    const textLooksShort = chosen.text.length < 48
    if (recordingLooksLong && textLooksShort && fullRecording.size > 0) {
      try {
        const fallbackText = (await transcribeAudio(fullRecording)).trim()
        if (fallbackText.length > chosen.text.length) {
          chosen = { text: collapseRepeatedPhrases(fallbackText), usedFallback: true }
        }
      } catch {
        // keep chosen
      }
