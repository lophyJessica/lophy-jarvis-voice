...
const asrStreamBase = () => `${getApiBase()}/p/jarvis/asr-stream`
...
function normalizeAsrText(text: unknown): string {
  ...
  return pieces.reduce((acc, piece) => {
    ...
    if (acc.includes(piece)) return acc
    if (piece.includes(acc)) return piece
    const sep = /[，。！？、；：,\.!?;:\s]$/.test(acc) ? '' : '，'
    return `${acc}${sep}${piece}`
  }, '')
}
