class JarvisPcmCaptureProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    // ~320ms @ 16kHz Int16 mono：百度 MID 需足够音频（v78 可靠配置）
    this.batchBytes = 10_240
    ...
    this.port.onmessage = (event) => {
      if (event.data?.type === 'config' && Number.isFinite(event.data.batchBytes)) {
        const nextBatchBytes = Math.max(320, Math.floor(event.data.batchBytes / 2) * 2)
        if (nextBatchBytes !== this.batchBytes) {
          this.batchBytes = nextBatchBytes
          this.resetBatch()
        }
      }
      ...
      if (event.data?.type === 'flush_preroll') {
        ...
        this.prerollChunks = []
        this.prerollBytes = 0
        if (!this.live) this.preroll = true
        this.resetBatch()
      }
