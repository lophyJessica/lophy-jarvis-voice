...
/**
 * 百度实时 ASR：MID/FIN 是「一句话」级别，不是整段累积。
 * 同句修订 → 更新 hypothesis；新句 → 把旧 hypothesis 提交进 committed。
 */
function isSameSentenceRevision(hypothesis: string, incoming: string): boolean {
  if (!hypothesis || !incoming) return true
  if (incoming === hypothesis) return true
  if (incoming.startsWith(hypothesis) || hypothesis.startsWith(incoming)) return true
  if (hypothesis.includes(incoming) || incoming.includes(hypothesis)) return true
  return similarityByShorter(hypothesis, incoming) >= 0.45
}
...
function applyStreamPiece(
  ...
  if (!hypothesis) {
    // 若整段已提交内容包含该片，忽略；否则作为当前句假设
    if (committed && (committed.endsWith(incoming) || committed.includes(incoming))) {
      return { committed, hypothesis: '' }
    }
    return { committed, hypothesis: incoming }
  }

  if (isSameSentenceRevision(hypothesis, incoming)) {
    if (incoming.startsWith(hypothesis) || incoming.length >= hypothesis.length) {
      return { committed, hypothesis: incoming }
    }
    return { committed, hypothesis }
  }
  ...
  // 新句子开始：固化上一句
  const nextCommitted = committed.endsWith(hypothesis) ? committed : `${committed}${hypothesis}`
...
export function useStreamingAsr({ onInterimText }: UseStreamingAsrOptions) {
  ...
  const queueChunkUpload = useCallback((blob: Blob) => {
    ...
    uploadQueueRef.current = uploadQueueRef.current.catch(() => undefined).then(async () => {
      ...
      try {
        ...
        const nextText = `${nextState.committed}${nextState.hypothesis}`
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    // 较长录音始终尝试整段 /asr，与流式结果取更长（本地需代理 /asr）
