...
function VoiceConsole({ username, onLogout, isDev }: { username: string; onLogout: () => void; isDev: boolean }) {
  ...
  const [vadThreshold, setVadThreshold] = useState(0.026)
  ...
  const processRecording = useCallback(async (blob: Blob) => {
    try {
      ...
      if (modeRef.current === 'voice' && autoModeRef.current) prepareStreamingAsr()
    } catch (error) {
    ...
    <main className="jarvis-shell">
      ...
      <Card className="conversation-panel" variant="borderless">
        ...
        <footer className="composer">
          {isVoiceMode && (streamingTranscript || transcript || status === 'recording' || status === 'transcribing') && (
            <div className="live-transcript"><span />{streamingTranscript || transcript || (status === 'transcribing' ? '识别收尾…' : '正在聆听…')}</div>
          )}
          {docUploads.length > 0 && (
