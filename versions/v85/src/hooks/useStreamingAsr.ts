...
/** 首包预览：过小 webm 易空返，多轮浪费 2s+ */
const webmPreviewFirstLaunchBytes = 2_400
const webmPreviewFirstMinBytes = 400
/** 累积 webm 增长超过此值且上一轮预览已结束 → 链式 /asr */
const webmPreviewGrowthBytes = 450
/** 空返后最多紧急重试次数（避免 9s 首字） */
const maxEmptyPreviewRetries = 2
const livePreviewLoopMs = 480
const livePreviewFirstDelayMs = 0
const livePreviewEarlyProbeMs = 220
...
function chooseFinalText(
  ...
  // 百度 end 有结果时以 end 为准；仅当 end 明显短于预览时才合并补前缀
  if (cleanEnd) {
    if (!previewBest || cleanEnd.length >= previewBest.length * 0.72) {
      return { text: cleanEnd, usedFallback: false }
    }
    const merged = clean(mergeInterimText(cleanEnd, previewBest))
    return {
      text: merged.length > cleanEnd.length ? merged : cleanEnd,
      usedFallback: merged !== cleanEnd,
    }
  }
  ...
  const requestWebmPreview = useCallback((blob: Blob, urgent = false) => {
    ...
    void transcribeAudio(previewBlob, controller.signal).then((text) => {
      ...
      if (!trimmed) {
        ...
        if (
          latest
          && latest.size > previewBlob.size + 400
