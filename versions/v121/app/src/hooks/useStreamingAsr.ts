...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    if (preferWebmStreamingCapture()) {
      ...
      const recordingBlob = fullRecording.size > 0
        ? fullRecording
        : (latestWebmRef.current ?? fullRecording)
      let text = ''
      if (recordingBlob.size > 0) {
        try {
          text = correctAsrText(collapseRepeatedPhrases((await transcribeAudio(recordingBlob)).trim()))
        } catch {
          text = ''
        }
      }
      if (text && generationRef.current === generation) {
