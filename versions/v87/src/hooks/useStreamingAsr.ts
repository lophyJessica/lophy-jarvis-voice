...
function chooseFinalText(
  ...
  const previewBest = [interimText, bestInterimText].map(clean).filter(Boolean)
    .reduce((a, b) => (a.length >= b.length ? a : b), '')
  const cleanFull = clean(fullAsrText)

  const pieces = [cleanEnd, previewBest, cleanFull].filter(Boolean)
  if (pieces.length === 0) return { text: '', usedFallback: true }

  let merged = pieces[0] ?? ''
  for (let index = 1; index < pieces.length; index += 1) {
    const piece = pieces[index] ?? ''
    if (!piece) continue
    const head = merged.slice(0, Math.min(28, merged.length))
    const sameUtterance = similarityByShorter(merged, piece) >= 0.32
      || (head.length >= 8 && piece.includes(head))
      || piece.startsWith(head)
      || merged.startsWith(piece.slice(0, Math.min(28, piece.length)))
    if (sameUtterance) merged = clean(mergeInterimText(merged, piece))
  }
  const longest = pieces.reduce((a, b) => (a.length >= b.length ? a : b))
  const best = merged.length >= longest.length ? merged : longest
  const usedFallback = !cleanEnd || clean(cleanEnd) !== best
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    if (fullRecording.size >= finalAsrMinBytes) {
      try {
        fullAsrRaw = (await transcribeAudio(fullRecording)).trim()
      } catch {
        // end + 预览仍可作为结果
      }
    }

    let chosen = chooseFinalText(
      finalText.trim(),
      interimText,
      bestInterimText,
      fullAsrRaw,
    )
    const cleanEnd = correctAsrText(collapseRepeatedPhrases(finalText.trim()))
    const cleanFull = correctAsrText(collapseRepeatedPhrases(fullAsrRaw))
    let finalSource: StreamingAsrResult['finalSource'] = finalText.trim()
      ? 'baidu-end'
      : (chosen.text ? 'webm-preview' : '')
    if (cleanFull.length > cleanEnd.length + 8) finalSource = 'fallback-asr'
    else if (!finalText.trim() && chosen.text) finalSource = 'webm-preview'
    ...
    if (!chosen.text && fullAsrRaw) {
      chosen = {
        text: correctAsrText(collapseRepeatedPhrases(fullAsrRaw)),
        usedFallback: true,
      }
