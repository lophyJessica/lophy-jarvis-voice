...
/** 播放时向前预取的句子数（首音打出后拉满，压低句间空档） */
const PREFETCH_DEPTH = 5
/** 冷启动：首音前最多并行拉几句（过大抢首包，过小句间停顿） */
const COLD_PREFETCH_DEPTH = 2
/** 同时进行的 /tts 请求上限 */
...
export function useSpeechSynthesis() {
  ...
  const fetchTtsBlobRaw = useCallback(async (text: string, signal: AbortSignal) => {
    ...
    try {
      // APK 必须走 getApiBase()（https://pmlophy.com）；浏览器为 ''，仍用相对 /tts
  ...
  const fillPrefetch = useCallback((sessionId: number) => {
    ...
    // 冷启动拉 2 句；开播后拉满，减少句间等待
    const depth = playbackStartedRef.current ? PREFETCH_DEPTH : COLD_PREFETCH_DEPTH
  ...
  /** 播放期间把队头预取等到 ready，减少句间空档 */
  const warmNextPrefetch = useCallback(async (sessionId: number) => {
    fillPrefetch(sessionId)
    const entry = prefetchQueueRef.current[0]
    if (!entry || entry.status === 'ready' || entry.status === 'failed') return
    await awaitPrefetchBlob(entry, sessionId)
  }, [awaitPrefetchBlob, fillPrefetch])
  ...
  const pumpQueue = useCallback(async (sessionId: number) => {
    ...
    try {
      while (sentenceQueueRef.current.length > 0 && sessionIdRef.current === sessionId) {
        ...
        // 播放当前句的同时预热下一句，句间尽量零等待
        const playPromise = playBlob(next.blob, sessionId)
        fillPrefetch(sessionId)
        void warmNextPrefetch(sessionId)
        await playPromise
      }
    } finally {
      pumpRunningRef.current = false
  ...
  }, [abortAllPrefetch, fillPrefetch, playBlob, resolveDrainWaiters, takeNextReadyBlob, warmNextPrefetch])
  ...
  const appendAssistantSpeechText = useCallback((chunk: string) => {
