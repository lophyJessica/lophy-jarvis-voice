...
/** webm 累积 /asr 预览：跟嘴主路径（chunk 无 MID，靠此跟嘴） */
const webmPreviewIntervalMs = 380
const webmPreviewIntervalWithInterimMs = 720
const webmPreviewMinBytes = 900
/** 首包预览：对齐 v67 整段 /asr 最小体积，减少空返 */
const webmPreviewFirstLaunchBytes = 3_200
const webmPreviewFirstMinBytes = 400
/** 累积 webm 增长超过此值且上一轮预览已结束 → 链式 /asr */
const webmPreviewGrowthBytes = 500
/** 空返后仅允许 1 次紧急重试 */
const maxEmptyPreviewRetries = 1
const livePreviewLoopMs = 500
const livePreviewFirstDelayMs = 0
...
function chooseFinalText(
  ...
  const cleanEnd = clean(finalText)
  const previewCandidates = [interimText, bestInterimText].map(clean).filter(Boolean)
  const previewBest = previewCandidates.reduce(
    (a, b) => (a.length >= b.length ? a : b),
    '',
  )

  // 百度 end 为主；预览更长且同源时合并补句尾（避免 end 截成「TLS一点」）
  if (cleanEnd) {
    if (!previewBest) return { text: cleanEnd, usedFallback: false }
    const merged = clean(mergeInterimText(cleanEnd, previewBest))
    const sameUtterance = similarityByShorter(cleanEnd, previewBest) >= 0.48
      || previewBest.startsWith(cleanEnd.slice(0, Math.min(36, cleanEnd.length)))
      || cleanEnd.startsWith(previewBest.slice(0, Math.min(36, previewBest.length)))
    if (sameUtterance && previewBest.length > cleanEnd.length + 8) {
      const best = merged.length >= previewBest.length ? merged : previewBest
      return { text: best.length >= cleanEnd.length ? best : cleanEnd, usedFallback: best !== cleanEnd }
    }
    return { text: cleanEnd, usedFallback: false }
  }
  ...
  return { text: best, usedFallback: true }
}
...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  const lastPreviewBlobSizeRef = useRef(0)
  const firstPreviewDoneRef = useRef(false)
  const emptyPreviewRetriesRef = useRef(0)
  ...
  const resetSessionState = useCallback((clearDisplay = false) => {
    ...
    lastPreviewBlobSizeRef.current = 0
    firstPreviewDoneRef.current = false
    emptyPreviewRetriesRef.current = 0
  ...
  const requestWebmPreview = useCallback((blob: Blob, urgent = false) => {
    const awaitingFirst = !firstPreviewDoneRef.current
    const minBytes = awaitingFirst
      ? webmPreviewFirstLaunchBytes
      : (urgent ? webmPreviewFirstMinBytes : webmPreviewMinBytes)
    if (!blob.size || blob.size < minBytes) return false
    ...
    if (webmPreviewInFlightRef.current) {
      pendingPreviewRef.current = true
      return false
    }
    if (!urgent && !awaitingFirst && now - lastWebmPreviewAtRef.current < minInterval) return false
    if (!awaitingFirst) lastWebmPreviewAtRef.current = now
    lastPreviewBlobSizeRef.current = blob.size
    ...
    emitDebugStats()
    void transcribeAudio(previewBlob, controller.signal).then((text) => {
      webmPreviewInFlightRef.current = false
      if (controller.signal.aborted || generationRef.current !== generation) return
      const trimmed = correctAsrText(text.trim())
      if (!trimmed) {
        const latest = latestWebmRef.current
        if (
          latest
          && latest.size > previewBlob.size + 800
          && emptyPreviewRetriesRef.current < maxEmptyPreviewRetries
        ) {
          emptyPreviewRetriesRef.current += 1
          requestWebmPreview(latest, true)
        } else {
          pendingPreviewRef.current = true
        }
        return
      ...
      emptyPreviewRetriesRef.current = 0
      if (!firstPreviewDoneRef.current) {
        firstPreviewDoneRef.current = true
        lastWebmPreviewAtRef.current = performance.now()
      }
      applyPreviewText(trimmed)
      const latest = latestWebmRef.current
      if (
        latest
        && latest.size >= webmPreviewMinBytes
        && latest.size > lastPreviewBlobSizeRef.current + webmPreviewGrowthBytes
      ) {
        requestWebmPreview(latest, false)
      } else if (pendingPreviewRef.current && latest) {
        pendingPreviewRef.current = false
        requestWebmPreview(latest, false)
      }
    }).catch(() => {
  ...
  const tryScheduleWebmPreview = useCallback((urgent = false) => {
    ...
    if (awaitingFirst) {
      if (blob.size < webmPreviewFirstLaunchBytes) return
      requestWebmPreview(blob, true)
      return
    }
    if (!urgent && blob.size <= lastPreviewBlobSizeRef.current + webmPreviewGrowthBytes) return
    requestWebmPreview(blob, urgent)
  }, [requestWebmPreview])
  ...
  }, [beginSpeechTiming, tryScheduleWebmPreview])
  ...
  const enqueueWebmSnapshot = useCallback((blob: Blob) => {
    ...
    tryScheduleWebmPreview(!firstPreviewDoneRef.current)
  }, [tryScheduleWebmPreview])

  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    const generation = generationRef.current
    // 勿先 abort：等最后一轮 /asr 预览落地，避免句尾被截成「建议什么使用」
    await waitForPreviewIdle(() => webmPreviewInFlightRef.current, 2_500)
    ...
    if (
      finalText.trim()
      && chosen.text.length < bestInterimText.length - 10
      && fullRecording.size >= finalAsrMinBytes
    ) {
      try {
        const fullAsr = correctAsrText(collapseRepeatedPhrases((await transcribeAudio(fullRecording)).trim()))
        if (fullAsr.length > chosen.text.length) {
          chosen = chooseFinalText(finalText.trim(), fullAsr, bestInterimText)
    ...
    } else if (!finalText.trim() && chosen.text) {
      finalSource = 'webm-preview'
    }
