...
import {
  ...
  composeMessageTextWithDocuments,
  isDocumentFile,
  isImageFile,
  MAX_DOCUMENT_BYTES,
...
function VoiceConsole({ username, onLogout, isDev }: { username: string; onLogout: () => void; isDev: boolean }) {
  ...
  const sendMessage = useCallback(async (overrideText?: string, overrideImages?: string[], overrideDocuments?: PendingDocument[]) => {
    ...
    const text = composeMessageTextWithDocuments(overrideText ?? input, documents)
  ...
  const hasPendingAttachments = pendingImages.length > 0 || pendingDocuments.length > 0
  const docParsingInProgress = docUploads.length > 0
  const primaryAction: 'send' | 'voice' | 'close' = (hasInputText || hasPendingAttachments) ? 'send' : isVoiceMode ? 'close' : 'voice'
