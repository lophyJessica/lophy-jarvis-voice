...
function VoiceConsole({ username, onLogout, isDev }: { username: string; onLogout: () => void; isDev: boolean }) {
  ...
  }, [appendStreamingSpeechText, beginStreamingSpeech, cancelSpeech, endStreamingSpeech, input, messageApi, pendingImages, persistTurn, synthesisSupported, transitionTo, waitForSpeechDrain])
  ...
  const processDocumentPaste = useCallback(async (files: File[]) => {
    for (const file of files) {
      ...
      try {
        ...
        if (!result.text.trim()) {
          void messageApi.warning('该 PDF 无文字层（可能是扫描件），暂不支持')
          continue
        }
        const analysisText = buildDocumentAnalysisMessage(result.filename, result.text, result.truncated)
        await sendMessage(analysisText, [])
      } catch (error) {
        const fallback = error instanceof Error ? error.message : '文档解析失败，请重试'
        void messageApi.error(fallback)
      } finally {
            ...
            <div className="composer-doc-upload-list" aria-live="polite">
              {docUploads.map((upload) => (
                <div key={upload.id} className="composer-doc-upload">
                  ...
                  <span>{`📄 正在解析 ${upload.filename}…`}</span>
                </div>
              ))}
            </div>
          )}
