...
export function useStreamingAsr({ onInterimText, onDebugStats }: UseStreamingAsrOptions) {
  ...
  /** 开口重置跟嘴计时；实时跟嘴由 PCM chunk MID 驱动 */
  ...
  /** 仅缓存 webm；流式失败时由 requestWebmPreview 兜底 */
  const enqueueWebmSnapshot = useCallback((blob: Blob) => {
    latestWebmRef.current = blob
    if (streamFailedRef.current && !streamLiveActiveRef.current) {
  ...
  const finishSession = useCallback(async (fullRecording: Blob): Promise<StreamingAsrResult> => {
    ...
    if (endClean) {
      const interimBest = correctAsrText(collapseRepeatedPhrases(bestInterimText))
      if (interimBest.length > endClean.length + 12) {
        chosen = chooseFinalText(endClean, interimBest, '')
        finalSource = 'baidu-end'
      } else {
        chosen = { text: endClean, usedFallback: false }
        finalSource = 'baidu-end'
      }
    } else {
      chosen = chooseFinalText('', interimText, bestInterimText)
      finalSource = streamWorked && chosen.text ? 'baidu-stream' : (chosen.text ? 'webm-preview' : '')
    }
