...
function VoiceConsole({ username, onLogout, isDev }: { username: string; onLogout: () => void; isDev: boolean }) {
  ...
  const writeClipboard = useCallback(async (text: string) => {
    ...
    try {
      if (!await copyTextToClipboard(text)) throw new Error('Clipboard write was rejected')
  ...
  const handleMessageListClick = useCallback((event: ReactMouseEvent<HTMLElement>) => {
    ...
    void writeClipboard(code).then((copied) => {
      if (!copied) return
          ...
          <div className="composer-actions">
            <Tooltip title={input.trim() ? '复制输入内容' : '无可复制内容'}>
              <Button
                ...
                onClick={() => void copyText(input, 'composer')}
                disabled={!input.trim()}
                aria-label="复制输入内容"
              />
            </Tooltip>
