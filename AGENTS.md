...
## 当前状态（2026-08-02，v13 融合版）

- ✅ VAD 免按钮 + 打断
- ✅ AudioWorklet PCM 直采 + 百度流式实时出字
- ✅ 跨浏览器记忆（共用 8868）+ Dexie（robin-console）本地兜底
- ✅ markdown 渲染 + 代码块复制 + 消息复制
- ✅ Edge TTS 播报（仅语音模式朗读，读取文本部分）
- ✅ 品牌统一"罗宾（Robin）"
- ✅ 文字模式：图片粘贴/上传（content[] 多模态）+ 长文展开
- ✅ 模式切换：ChatGPT 式三态按钮（发送/语音/退出）+ robin-mode 持久化
- ✅ 手机端 F09：单列 + 安全区
