...
## 当前状态（2026-08-07，v157）

| 版本 | 摘要 |
|------|------|
| v157 | 菲儿头像本地化至 `public/robin-avatar.png`，APK WebView 不再依赖远程图片 |
| v156 | 罗宾头像换成菲儿（灰泰迪）插画；主界面与登录页统一 |
| v155 | 完成文字版播报优化，豆包参与一轮对话，罗宾接力 |
| v154 | 版本号从左下角移到顶栏状态标签同排右侧（Hermes/同步旁） |
| v153 | TTS 播报前剥离 markdown 代码围栏；仅代码块无正文则跳过播报 |
| v152 | 流式 delta 仅用于 assistant 专用上屏；自动 TTS 改为回复完成后只从 assistant 消息生成，并过滤当前用户指令回声 |
| v151 | 自动 TTS 仅消费助手角色的流式 delta/完整回复，拒绝用户角色内容，避免输入框指令进入 `/tts` |
| v150 | TTS 流式队列明确只接收助手回复 delta；手动重播与预热只取 messages 中最新 assistant 消息，排除全部表单控件文本 |
